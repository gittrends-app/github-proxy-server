...
export function createCli() {
    ...
    return program
        ...
        .default(3000)
        .env('PORT'))
        .addOption(new Option('-t, --token [token]', 'GitHub token to be used')
        ...
        .addOption(new Option('--min-remaining <number>', 'Stop using token on a minimum of')
        .argParser(Number)
        .default(100)
        .env('GPS_MIN_REMAINING'))
        .addOption(new Option('--clustering', '(clustering) enable clustering (requires redis)')
        .default(false)
        .env('GPS_CLUSTERING_HOST'))
        .addOption(new Option('--clustering-host [host]', '(clustering) redis host')
        .implies({ clustering: true })
        .default('localhost')
        .env('GPS_CLUSTERING_HOST'))
        .addOption(new Option('--clustering-port [port]', '(clustering) redis port')
        .argParser(Number)
        .implies({ clustering: true })
        .default(6379)
        .env('GPS_CLUSTERING_PORT'))
        .addOption(new Option('--clustering-db [db]', '(clustering) redis db')
        .argParser(Number)
        .implies({ clustering: true })
        .default(0)
        ...
        const server = app.listen({ host: '0.0.0.0', port: options.port }, (error) => {
            ...
            consola.success(`Proxy server running on ${host} (tokens: ${chalk.greenBright(tokens.length)})`);
            function formatObject(object) {
                return Object.entries(omitBy(object, (value) => isNil(value)))
                    .sort((a, b) => (a[0] > b[0] ? 1 : -1))
                    .map(([k, v]) => `${k}: ${isObjectLike(v)
                    ? `{ ${formatObject(v)} }`
                    : chalk.greenBright(v)}`)
                    .join(', ');
            }
            consola.success(`${chalk.bold('Options')}: %s`, formatObject(omit(appOptions, ['token', 'tokens'])));
        });
